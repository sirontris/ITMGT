...
def savings(gross_pay, tax_rate, expenses):
    # Apply the tax rate to the gross pay of the employee; round down
    after_tax = gross_pay*(1-tax_rate) // 1
    
    # Subtract the expenses from the after-tax pay of the employee
    
    after_tax_and_expenses = int(after_tax - expenses)
    
    return after_tax_and_expenses
...
def body_mass_index(weight, height):
    ...
    converted_height = ((height[0]*12 + height[1])*2.54)/100
    
    BMI = float(converted_weight / (converted_height ** 2))
